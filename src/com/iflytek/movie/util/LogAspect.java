package com.iflytek.movie.util;
...
public class LogAspect {
	...
	// 增强处理：在目标方法执行之前植入增强
	@Before("execution(* com.iflytek.movie.service.impl.*.*(..))")
	public void beforeDo() {
		System.out.println("------------>Before");
	}

	// 增强处理：在目标方法结束后植入增强（不管成功或异常终止）
	@After("execution(* com.iflytek.movie.service.impl.*.*(..))")
	public void afterDo() {
	...
	@AfterReturning(returning = "result", pointcut = "execution(* com.iflytek.movie.service.impl.*.*(..))")
	...
	@AfterThrowing(throwing = "ex", pointcut = "execution(* com.iflytek.movie.service.impl.*.*(..))")
	public void afterThrowingDo(Throwable ex) {
		System.out.println("------------>AfterThrowing，异常：" + ex);
	}

	// 增强处理：Before和AfterReturning的总和
	@Around("execution(* com.iflytek.movie.service.impl.*.*(..))")
