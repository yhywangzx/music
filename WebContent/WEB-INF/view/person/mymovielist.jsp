...
<body>
	...
	<form action="person/mymovielist" method="post">
		<div class="div">
			<div class="scmusic">
				<a href="person/uploadmovie">上传电影</a>
			</div>
			<table id="ListArea" border="0" class="t1 table">
				...
				<c:forEach var="item" items="${ requestScope.movieList }" varStatus="st">
					<tr>
						<td>${ st.index + 1 }</td>
						<td>${ item.mname }</td>
						<td>${ item.director }</td>
						...
						<a href="person/updatemovie?id=${ item.id }">编辑</a>
						<a href="download?id=${ item.id }">下载</a>
						<a href="delete?id=${ item.id }" class="delete">删除</a></td>
					</tr>
				</c:forEach>
